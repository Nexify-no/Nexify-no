...
export function LoginPage() {
  ...
  // "Sist brukt" hint: remember the last login method (so users with several
  // accounts pick the right one). Stored locally on this device only.
  const [lastMethod, setLastMethod] = useState<string | null>(null);
  useEffect(() => {
    try { setLastMethod(localStorage.getItem(LAST_LOGIN_KEY)); } catch { /* ignore */ }
  }, []);
  const rememberMethod = (m: string) => {
    try { localStorage.setItem(LAST_LOGIN_KEY, m); } catch { /* ignore */ }
  };
            ...
            {mode !== "forgot" && (
              <>
                ...
                <Button
                  ...
                  size="lg"
                >
                  <a href="/api/auth/login/google" onClick={() => rememberMethod("google")}>
                    ...
                    Logg inn med Google
                    {lastMethod === "google" && (
                      <span className="ml-2 rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-semibold text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300">Sist brukt</span>
                    )}
                  </a>
                </Button>

                {/* LinkedIn login - backend route: GET /api/auth/login/linkedin */}
                <Button
                  asChild
                  variant="outline"
                  className="w-full h-12 text-base font-medium border-gray-300"
                  size="lg"
                >
                  <a href="/api/auth/login/linkedin" onClick={() => rememberMethod("linkedin")}>
                    <svg className="mr-3 h-5 w-5" viewBox="0 0 24 24" fill="#0A66C2" aria-hidden="true">
